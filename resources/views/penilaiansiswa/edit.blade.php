...
<div class="container-fluid px-4">
           <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Tambah </span> Penilaian</h4>
            <div class="card mb-4">
...
<form action="{{ route('penilaiansiswa.update', $siswa->id) }}" method="POST">
    ...
    @foreach ($kriteria as $k)
        <div class="mb-3">
            ...
            <select name="penilaiansiswa[{{ $k->id }}]" class="form-control">
                @foreach ($k->sub_kriteria as $sub)
                    <option value="{{ $sub->id }}"
                        {{ (isset($penilaiansiswa[$k->id]) && $penilaiansiswa[$k->id]->sub_kriteria_id == $sub->id) ? 'selected' : '' }}>
